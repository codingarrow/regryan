...
class redux_auth
{
	 ...
	 * @author Mathew
	 **/
	public function forgotten_password($email, $use_email = false)
	{
        $from             			= $this->ci->config->item('from');
        $from_email       			= $this->ci->config->item('from_email');
        $reply            			= $this->ci->config->item('reply');
        $reply_email      			= $this->ci->config->item('reply_email');
		$forgotten_password = $this->ci->redux_auth_model->forgotten_password($email);

		if ($forgotten_password)
		{
			// Get user information.
			$profile = $this->ci->redux_auth_model->profile($email, $use_email);
            ...
            $this->ci->email->reply_to($reply_email, $reply);
	 ...
	 * @author Mathew
	 **/
	public function forgotten_password_complete($code, $use_email = false)
	{
        $from             			= $this->ci->config->item('from');
        $from_email       			= $this->ci->config->item('from_email');
        $reply            			= $this->ci->config->item('reply');
	    ...
	    $profile                  	= $this->ci->redux_auth_model->profile($code, $use_email);
